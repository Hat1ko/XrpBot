...
public final class AccountLastIdConverter {
	...
	public static AccountLastIdEntity toEntity(AccountLastIdDTO accountLastIdDTO) {
		return AccountLastIdEntity.builder().publicKey(accountLastIdDTO.getPublicKey())
				.lastId(accountLastIdDTO.getLastId()).lastLedger(accountLastIdDTO.getLastLedger()).build();
	}
